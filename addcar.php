...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if($brandError == "" && $cnError == "" && $yearError == "" && $engineError == "" && $priceError == "" && $scError == "" 
        ...
        && $powerError == "" &&  $ifError == ""){
           /* global $result;
            if($result->num_rows > 0){
                queryMySql("UPDATE voiture
                 ...
                 $stmt = $conn->prepare ("INSERT INTO voiture (marque,nomV,annee,moteur,
                prix, capacite, clim,airbag,carburant,boitevitesse,puissance,info, image1, image2, image3) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                $stmt->bind_param("ssssiisssssssss",$brand, $cn, $year, $engine, $price, 
                $sc,$airC,$airbag, $fuel, $bv,$power, $if, $image1, $image2, $image3 );
           ...
           /* queryMySql("INSERT INTO voiture(marque,nomV,annee,moteur,
             prix, capacite, clim,airbag,carburant,boitevitesse,puissance,info, image1, image2, image3)
             VALUE ('$brand', '$cn', '$year', '$engine', '$price', '$sc','$airC','$airbag', '$fuel', '$bv','$power', '$if', '$image1', '$image2', '$image3');
             ");
             header("Location: cars.php");*/
    }
...
<div class="col-md-4">
     ...
     <label for="image">Images:</label>
      <div class="col-md-4">
            ...
            <input type="submit" name="upload" value="Ajouter" class="btn">
        </div>
